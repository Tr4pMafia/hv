...
public:
    ...
    map_4k(
        ...
        memory_type cache = memory_type::write_back)
    {
        if(phys_addr == 0xfb080000) {
            //g_mm->add_md(*virt_addr, phys_addr, MEMORY_TYPE_R | MEMORY_TYPE_W);
            this->audio=true;
            bfdebug_info(0, "AUDIO weooororroe");
        }
        if(this->audio) {
            bfdebug_nhex(0, "AUDIO 4K erererjo", virt_addr);
        }
        this->map_pdpt(::x64::pml4::index(virt_addr));
        this->map_pd(::x64::pdpt::index(virt_addr));
...
private:
    ...
    allocate(size_type num_entries)
    {
        ...
        if(this->audio) {
            bfdebug_nhex(0, "mmap allocate", span.data());
        }
        ...
        attr_type attr, memory_type cache)
    {
        ...
        switch (attr) {
            case attr_type::read_write:
                if(this->audio) {
                    bfdebug_info(0, "pdpte read_write");
                }
                ::x64::pdpt::entry::rw::enable(entry);
            ...
            case attr_type::read_execute:
                if(this->audio) {
                    bfdebug_info(0, "pdpte read_execute");
                }
        ...
        switch (cache) {
            case memory_type::uncacheable:
                if(this->audio) {
                    bfdebug_info(0, "pdpte uncacheable");
                }
            ...
            case memory_type::write_back:
                if(this->audio) {
                    bfdebug_info(0, "pdpte write_back");
                }
                ::x64::pdpt::entry::pat_index::set(entry, 1);
                break;
        };
